use super::Frame;

const BASE_FRAME: [u8; 394] = [
    0, 2, 126, 126, 24, 60, 102, 66, 0, 1, 12, 28, 112, 112, 28, 12, 0, 1, 116, 116, 20, 20, 124,
    104, 0, 1, 124, 124, 0, 1, 112, 120, 44, 36, 124, 124, 0, 26, 128, 64, 64, 32, 32, 32, 32, 16,
    16, 16, 16, 16, 8, 8, 4, 4, 4, 8, 48, 64, 128, 0, 46, 128, 128, 128, 0, 4, 192, 96, 48, 24, 12,
    132, 198, 98, 35, 51, 17, 145, 113, 241, 113, 145, 17, 51, 35, 98, 198, 132, 12, 24, 48, 96,
    192, 0, 18, 24, 100, 130, 2, 2, 2, 2, 2, 1, 0, 4, 128, 128, 0, 9, 128, 0, 1, 48, 48, 0, 1, 192,
    193, 193, 194, 4, 8, 16, 32, 64, 128, 0, 3, 128, 128, 128, 128, 64, 64, 64, 64, 32, 32, 32, 32,
    16, 16, 16, 16, 8, 8, 8, 8, 8, 196, 4, 196, 4, 196, 2, 194, 2, 194, 1, 1, 1, 1, 0, 5, 252, 15,
    1, 0, 1, 248, 14, 31, 109, 140, 148, 148, 164, 166, 249, 224, 255, 224, 249, 166, 164, 148,
    148, 140, 109, 31, 14, 248, 0, 1, 1, 15, 252, 0, 15, 192, 56, 4, 3, 0, 7, 12, 12, 12, 13, 1, 0,
    1, 64, 160, 33, 34, 18, 17, 17, 17, 9, 8, 8, 8, 8, 4, 4, 8, 8, 16, 16, 16, 16, 16, 17, 15, 1,
    1, 0, 18, 170, 170, 255, 255, 195, 191, 127, 3, 127, 191, 195, 255, 255, 170, 170, 0, 6, 31,
    120, 192, 0, 1, 15, 56, 124, 219, 152, 20, 20, 18, 50, 207, 3, 255, 3, 207, 50, 18, 20, 20,
    152, 219, 124, 56, 15, 0, 1, 192, 120, 31, 16, 16, 16, 16, 8, 8, 8, 8, 8, 4, 4, 4, 4, 4, 2, 3,
    2, 2, 1, 1, 1, 1, 1, 1, 2, 2, 4, 4, 8, 8, 8, 8, 8, 7, 0, 42, 2, 130, 135, 31, 7, 159, 7, 28, 7,
    159, 7, 159, 7, 2, 130, 0, 4, 32, 16, 16, 16, 17, 11, 14, 12, 24, 16, 49, 35, 98, 102, 68, 68,
    71, 71, 71, 68, 68, 102, 98, 35, 49, 16, 24, 12, 6, 3, 1, 0, 78, 7, 8, 8, 23, 0, 1, 15, 1, 2,
    1, 15, 0, 1, 15, 2, 5, 8,
];
const REGIONS_0: [usize; 0] = [];
const DIFF_0: [u8; 0] = [];
const REGIONS_1: [usize; 0] = [];
const DIFF_1: [u8; 0] = [];
const REGIONS_2: [usize; 26] = [
    58, 69, 71, 74, 75, 79, 177, 181, 183, 187, 190, 192, 201, 202, 203, 205, 206, 216, 304, 308,
    315, 320, 323, 330, 334, 338,
];
const DIFF_2: [u8; 63] = [
    0, 1, 128, 128, 64, 64, 64, 64, 32, 32, 32, 32, 4, 2, 2, 24, 96, 128, 0, 1, 60, 194, 1, 1, 4,
    4, 2, 1, 0, 3, 96, 96, 129, 130, 130, 132, 8, 16, 32, 64, 128, 0, 1, 128, 112, 25, 6, 24, 24,
    24, 27, 3, 34, 36, 20, 18, 18, 18, 11, 5, 5, 9, 9,
];
const REGIONS_3: [usize; 8] = [51, 53, 69, 76, 177, 183, 304, 307];
const DIFF_3: [u8; 18] = [
    128, 128, 8, 4, 2, 1, 1, 2, 12, 30, 225, 0, 2, 1, 1, 128, 112, 12,
];
const REGIONS_4: [usize; 8] = [69, 70, 71, 77, 177, 181, 304, 307];
const DIFF_4: [u8; 14] = [8, 4, 2, 2, 2, 4, 56, 28, 226, 1, 1, 128, 112, 12];
const REGIONS_5: [usize; 20] = [
    51, 53, 69, 76, 177, 183, 185, 188, 206, 217, 304, 307, 310, 321, 322, 323, 336, 345, 441, 451,
];
const DIFF_5: [u8; 56] = [
    128, 128, 8, 4, 2, 1, 1, 2, 12, 30, 225, 0, 2, 1, 1, 129, 128, 128, 0, 1, 1, 225, 26, 6, 9, 49,
    53, 1, 138, 124, 128, 112, 12, 24, 6, 5, 152, 153, 132, 195, 124, 65, 65, 64, 32, 4, 4, 4, 4,
    2, 2, 2, 1, 1, 0, 10,
];
const REGIONS_6: [usize; 32] = [
    51, 53, 69, 76, 86, 90, 95, 97, 177, 183, 185, 188, 206, 210, 214, 218, 219, 227, 304, 307,
    310, 321, 322, 323, 346, 355, 416, 417, 441, 451, 477, 480,
];
const DIFF_6: [u8; 71] = [
    128, 128, 8, 4, 2, 1, 1, 2, 12, 248, 248, 248, 248, 128, 128, 30, 225, 0, 2, 1, 1, 129, 128,
    128, 0, 1, 1, 1, 2, 67, 135, 7, 1, 184, 188, 190, 159, 95, 95, 79, 76, 128, 112, 12, 24, 6, 5,
    152, 153, 132, 67, 124, 65, 65, 64, 32, 61, 124, 252, 252, 252, 252, 252, 60, 12, 63, 0, 10, 1,
    3, 3,
];
const REGIONS_7: [usize; 22] = [
    51, 53, 69, 76, 177, 183, 206, 217, 304, 307, 315, 319, 336, 345, 433, 441, 448, 452, 565, 572,
    576, 584,
];
const DIFF_7: [u8; 69] = [
    128, 128, 8, 4, 2, 1, 1, 2, 12, 30, 225, 0, 2, 1, 1, 0, 1, 1, 225, 26, 6, 9, 49, 53, 1, 138,
    124, 128, 112, 12, 0, 3, 1, 4, 4, 4, 4, 2, 2, 2, 1, 1, 122, 122, 121, 121, 121, 121, 57, 49,
    136, 136, 135, 128, 48, 120, 124, 254, 255, 63, 7, 255, 255, 127, 127, 63, 62, 28, 24,
];

use crate::compression;
pub(super) fn get_frame(frame: &Frame) -> Vec<u8> {
    let compact = match *frame {
        Frame::Idle(0) => (REGIONS_0.iter(), DIFF_0.iter()),
        Frame::Idle(1) => (REGIONS_1.iter(), DIFF_1.iter()),
        Frame::Idle(2) => (REGIONS_2.iter(), DIFF_2.iter()),
        Frame::Idle(3) => (REGIONS_3.iter(), DIFF_3.iter()),
        Frame::Idle(4) => (REGIONS_4.iter(), DIFF_4.iter()),
        Frame::Prep => (REGIONS_5.iter(), DIFF_5.iter()),
        Frame::Tap(0) => (REGIONS_6.iter(), DIFF_6.iter()),
        Frame::Tap(1) => (REGIONS_7.iter(), DIFF_7.iter()),
        _ => panic!("Unexpected frame requested"),
    };
    let regions = compact.0.cloned().collect::<Vec<_>>();
    let diff = compact.1.cloned().collect::<Vec<_>>();
    let original = compression::CompressedBytes::from_compressed(&BASE_FRAME);
    let diff = compression::CompressedDiff::from_regions_and_diff(&regions, &diff);
    diff.reconstruct_frame(&original)
}
